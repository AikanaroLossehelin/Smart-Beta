...
```{r echo=FALSE}
p <- ggplot(daily_crypto_stat, aes(x = Mean, y = SD)) +
...
p <- ggplot(volume, aes(x = volume)) +
  ...
  scale_x_log10() +
  geom_vline(xintercept = mean_volume, color = "darkgreen", linetype = "dashed") + 
...
for(i in 1:nrow(volume_stat)) {
  p <- p + geom_vline(aes(xintercept = volume_stat$Mean[i]), color = "red", linetype = "dashed") +
    annotate("text", x = volume_stat$Mean[i], y = 0.05, label = volume_stat$Coin[i], angle = 45, hjust = 0, vjust = 0, size = 3)
}
