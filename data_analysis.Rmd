...
rdt <- rdt_crypto %>%
  pivot_longer(
    -date, # Exclure la colonne de date du pivotement
    names_to = "currency", # Nom de la nouvelle colonne pour les monnaies
    values_to = "value" # Nom de la nouvelle colonne pour les capitalisations
  )
```
...
p <- ggplot(volume_stat, aes(x = Mean, y = SD)) +
  ...
  scale_y_log10() +
  theme_minimal() +
  labs(title = "Dispersion des Monnaies par moyenne et écart-type de leur volume", 
       x = "Moyenne", 
       y = "Écart-Type")
...
### Modélisation des volatilités conditionnelles des volumes

```{r echo=FALSE}
monnaie_cols <- colnames(volume_binance_daily)[-1]
...
for(i in seq(1, length(monnaie_cols), by = graphs_per_window)) {
  ...
  for(j in i:min(i+graphs_per_window-1, length(monnaie_cols))) {
    mvbd <- mean(volume_binance_daily[[monnaie_cols[j]]])
    ts_data <- ts((volume_binance_daily[[monnaie_cols[j]]] - mvbd)^2)
    Pacf(ts_data, main = paste("PACF pour", monnaie_cols[j]))
  }
}

```
On part sur un modèle GARCH(1,3)
